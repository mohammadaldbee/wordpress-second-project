  ...
  function webroom_wc_remove_password_strength() {
    ...
    function moppm_redirect_page()
    {
        ...
        if(get_site_option('moppm_plugin_redirect'))
        {
            ...
            wp_redirect(admin_url().'admin.php?page=moppm');
            exit();
        }
    ...
    function moppm_send_reset_link($email,$user_id,$user){
            ...
            $messages = '<table cellpadding="25" style="margin:0px auto">
                            ...
                            <p style="margin-top:0;margin-bottom:10px"><a href="' . network_site_url("wp-login.php?action=rp&key=$adt_rp_key&login=" . rawurlencode(sanitize_text_field($user_login)), 'login') . '">' . network_site_url("wp-login.php?action=rp&key=$adt_rp_key&login=" . rawurlencode(sanitize_text_field($user_login)), 'login') . ' </a> 
